...
class ArrayAgg<T>(expression: ExpressionWithColumnType<T>) : CustomFunction<Array<T>?>(
    functionName = "ARRAY_AGG",
    columnType = ArrayColumnType(expression.columnType),
    expr = arrayOf(expression),
)

fun <T : Any?> ExpressionWithColumnType<T>.arrayAgg(): ArrayAgg<T> = ArrayAgg(this)
