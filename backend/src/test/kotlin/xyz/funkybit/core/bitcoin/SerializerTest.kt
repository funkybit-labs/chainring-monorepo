...
class SerializerTest {
    ...
    fun `test instructions`() {
        ...
        assertEquals(
            ArchProgramBinaryFormat.encodeToByteArray(exchangeInstruction).toHex(),
            ArchProgramBinaryFormat.encodeToByteArray(ArchProgramBinaryFormat.decodeFromByteArray<ProgramInstruction>(ArchProgramBinaryFormat.encodeToByteArray(exchangeInstruction))).toHex(),
        )

        assertEquals(
            "000300666565070070726f6772616d03",
            ArchProgramBinaryFormat.encodeToByteArray(exchangeInstruction).toHex(false),
        )
        ...
        val instruction = ArchNetworkRpc.Instruction(
            ...
            data = ArchProgramBinaryFormat.encodeToByteArray(exchangeInstruction).toUByteArray(),
        )
        ...
        assertEquals(
            "640000007a787937",
            ArchProgramBinaryFormat.encodeToByteArray(addressIndex).toHex(false),
        )
        val deserialized = ArchProgramBinaryFormat.decodeFromByteArray<ProgramInstruction.AddressIndex>(ArchProgramBinaryFormat.encodeToByteArray(addressIndex))
        assertEquals(
            addressIndex.index,
            deserialized.index,
        )
    ...
    fun `test deserialize account state`() {
        ...
        val p = ArchProgramBinaryFormat.decodeFromByteArray<ArchAccountState.Program>(programStateBytes.toHexBytes())
        ...
        val t = ArchProgramBinaryFormat.decodeFromByteArray<ArchAccountState.Token>(tokenStateBytes.toHexBytes())
