...
do
  ...
  psql -U chainring -d chainring -h localhost -c "delete from erc20_token where symbol='$coin_type'; insert into erc20_token (guid, created_at, created_by, name, symbol, chain, address) values ('`uuidgen`', 'now()', 'local_init', '$coin_type Coin', '$coin_type', 'Ethereum', '$DEPLOYED_ADDRESS');"
done
