...
do
  ...
  PGPASSWORD=$PGPASSWORD psql -U chainring -d chainring -h localhost -c "delete from erc20_token where symbol='$coin_type'; insert into erc20_token (guid, created_at, created_by, name, symbol, chain, address, decimals) values ('`uuidgen`', 'now()', 'local_init', '$coin_type Coin', '$coin_type', 'Ethereum', '$DEPLOYED_ADDRESS', 18);"
done
